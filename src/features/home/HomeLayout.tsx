...
import { useLoansAfterDate } from "../loans/useLoansAfterDate";
...
export default function HomeLayout() {
 ...
 const {recentLoans,isFetching,err,numDays} = useLoansAfterDate()
  if(isLoading || isLoadingLoanees || isLoadingAgents || isFetching) return <Loader/>
  if(error || loaneeError || aErro || err) return <div>{err?.message}</div>
  if(!loans || !loanees || !agents || !recentLoans) return <div>Something went wrong</div>
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2">
            <HomeGraph loans={recentLoans} numDays={numDays} />
        </div>
    </>
  )
}
